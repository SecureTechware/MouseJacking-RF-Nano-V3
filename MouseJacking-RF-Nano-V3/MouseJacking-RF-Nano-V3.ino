...
// URL: https://github.com/SecureTechware/MouseJacking-RF-Nano-V3
...
uint8_t writeRegister(uint8_t registerAddress, const uint8_t* dataBuffer, uint8_t bufferLength) {
  uint8_t operationStatus;

  // Start SPI communication by setting CSN low
  digitalWrite(CSN, LOW);

  // Send the write command and register address
  operationStatus = SPI.transfer(W_REGISTER | (REGISTER_MASK & registerAddress));

  // Write each byte of the data buffer to the register
  while (bufferLength--)
    SPI.transfer(*dataBuffer++);

  // End SPI communication by setting CSN high
  digitalWrite(CSN, HIGH);
...
void log_checksum() {
  uint8_t checksum = 0xff; // Initialize checksum to maximum value
  int lastIndex = payload_size - 1; // Determine the last index in the payload

  // Compute checksum by iterating through all payload bytes except the last
  for (int index = 0; index < lastIndex; index++)
    checksum -= payload[index];

  // Adjust the checksum value
  checksum++;
  payload[lastIndex] = checksum; // Store the checksum in the last payload byte
}

void log_transmit(uint8_t meta, uint8_t keysToSend[], uint8_t numKeys) {
  // Initialize an empty payload
  payload_size = 10;
  for (int i = 0; i < payload_size; i++)
    payload[i] = 0;

  // Prepare the key down frame
  payload[1] = 0xC1; // Frame type identifier
  payload[2] = meta;

  // Add keys to the payload
  for (int j = 0; j < numKeys; j++)
    payload[3 + j] = keysToSend[j];

  // Calculate and append the checksum
  log_checksum();

  // Send the key down frame
  transmit();
  delay(5);

  // Prepare the key up (null) frame
  for (int k = 2; k < 9; k++)
    payload[k] = 0;
